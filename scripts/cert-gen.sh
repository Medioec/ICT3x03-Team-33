...
sign_domain_cert() {
    ...
    openssl x509 -req -in $certdir/$domain-csr.pem -CA $cadir/ca-cert.pem -CAkey $cadir/ca-key.pem -CAcreateserial -out $certdir/$domain-cert.pem -days 365 -extensions SAN -extfile <(cat /etc/ssl/openssl.cnf <(printf "\n[SAN]\nsubjectAltName=DNS:$domain"))
}
