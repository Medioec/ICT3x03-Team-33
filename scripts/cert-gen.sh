...
openssl req -new -x509 -SHA384 -nodes -key $cadir/ca-key.pem -out $cadir/ca-cert.pem -days 3650 -subj "/C=SG/ST=SG/L=SG/O=3x03/OU=Dragoons/CN=EricCA"
mkdir -p $certdir
sign_domain_cert() {
    domain=$1
    ...
    openssl req -new -SHA384 -nodes -key $certdir/$domain-key.pem -out $certdir/$domain-csr.pem -subj "/C=SG/ST=SG/L=SG/O=3x03/OU=Hackers/CN=$domain" -reqexts SAN -extensions SAN -config <(cat /etc/ssl/openssl.cnf <(printf "[SAN]\nsubjectAltName=DNS:$domain"))
